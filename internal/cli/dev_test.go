...
func TestRunDev_UsesExplicitFlags(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- runDev(app, "backend", 9090, "10.0.0.1", false, "", nil, cmd, WithEnvironmentDetector(detector))
	}()
...
func TestRunDev_TailscaleError(t *testing.T) {
	...
	err := runDev(app, "web", 0, "", false, "", nil, cmd, WithEnvironmentDetector(detector))
...
func TestRunDev_CollisionDifferentOwner(t *testing.T) {
	...
	err := runDev(app, "", 0, "", false, "", nil, cmd, WithEnvironmentDetector(detectorBob))
	require.Error(t, err)
