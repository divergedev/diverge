...
func TestRunDev_CreatesPreviewGroup(t *testing.T) {
	...
	go func() {
		errCh <- runDev(runDevParams{App: app, Cmd: cmd, NoTunnel: true, Options: []DevOption{WithEnvironmentDetector(detector)}})
	}()
...
func TestRunDev_UsesExplicitFlags(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- runDev(runDevParams{App: app, Service: "backend", Port: 9090, Endpoint: "10.0.0.1", Cmd: cmd, NoTunnel: true, Options: []DevOption{WithEnvironmentDetector(detector)}})
	}()
...
func TestRunDev_TailscaleError(t *testing.T) {
	...
	err := runDev(runDevParams{App: app, Service: "web", Cmd: cmd, NoTunnel: true, Options: []DevOption{WithEnvironmentDetector(detector)}})
...
func TestRunDev_CollisionDifferentOwner(t *testing.T) {
	...
	err := runDev(runDevParams{App: app, Cmd: cmd, NoTunnel: true, Options: []DevOption{WithEnvironmentDetector(detectorBob)}})
	require.Error(t, err)
