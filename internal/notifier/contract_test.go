...
func AssertHTTPContract(t *testing.T, req *http.Request, contract ContractAssertion) {
	...
	assert.Regexp(t, contract.PathPattern, path)
...
func TestContract_GitHubNotifier(t *testing.T) {
	...
	err = n.PostEnvironmentFailed(context.Background(), env, "failed")
	require.NoError(t, err)
	AssertHTTPContract(t, lastReq, patchContract)

	err = n.PostEnvironmentTeardown(context.Background(), env)
	require.NoError(t, err)
	AssertHTTPContract(t, lastReq, patchContract)

	err = n.UpdateEnvironmentStatus(context.Background(), env)
	require.NoError(t, err)
	AssertHTTPContract(t, lastReq, patchContract)
}
...
func TestContract_GitLabNotifier(t *testing.T) {
	...
	postContract := ContractAssertion{
		Method:      http.MethodPost,
		PathPattern: regexp.MustCompile(`^/api/v4/projects/group%2Fproject/merge_requests/99/notes$`),
		Headers: map[string]string{
	...
	putContract := ContractAssertion{
		...
		PathPattern: regexp.MustCompile(`^/api/v4/projects/group%2Fproject/merge_requests/99/notes/54321$`),
		Headers: map[string]string{
			"PRIVATE-TOKEN": "fake-token",
			"Content-Type":  "application/json",
		},
...
func TestContract_GitHubPreviewGroupNotifier(t *testing.T) {
	...
	err = n.PostGroupFailed(context.Background(), pg, "failed")
	require.NoError(t, err)
	AssertHTTPContract(t, lastReq, patchContract)

	err = n.PostGroupTeardown(context.Background(), pg)
	require.NoError(t, err)
	AssertHTTPContract(t, lastReq, patchContract)
...
func TestContract_GitLabPreviewGroupNotifier(t *testing.T) {
	var lastReq *http.Request
	var lastBody []byte
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"id": 22222}`))
	}))
	defer ts.Close()

	n := notifier.NewGitLabPreviewGroupNotifier(ts.URL, "fake-token")
	...
	postContract := ContractAssertion{
		Method:      http.MethodPost,
		PathPattern: regexp.MustCompile(`^/api/v4/projects/org%2Fteam%2Fproject/merge_requests/88/notes$`),
		Headers: map[string]string{
			"PRIVATE-TOKEN": "fake-token",
			"Content-Type":  "application/json",
		},
	...
	assert.Equal(t, int64(22222), pg.Status.CommentID)

	putContract := ContractAssertion{
		Method:      http.MethodPut,
		PathPattern: regexp.MustCompile(`^/api/v4/projects/org%2Fteam%2Fproject/merge_requests/88/notes/22222$`),
		Headers: map[string]string{
			"PRIVATE-TOKEN": "fake-token",
			"Content-Type":  "application/json",
		},
